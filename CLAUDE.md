...
| [`docs/combat_actions.md`](docs/combat_actions.md) | 🚩 **A blow that takes time.** The server owns a deadline, not an action — which is why a bow stands still for ten seconds and a spoiled swing has no way to say so. `CombatAction` as a committed, interruptible process on **three separated axes** — *kind* (what the impact does), *trigger* (a clock, a riding-through, a held aim), *rules* (what the world does to it in between) — with the round taken at the nock, interruption and fatigue as a declared condition/effect table rather than flags, and two new packets, because the *end* of an action never crossed the wire at all. Seven phases; the polearm, the joust and overwatch are the last three, and each is cheap only because the axes were separated first. |
