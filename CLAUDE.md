...
| [`docs/combat_actions.md`](docs/combat_actions.md) | 🚩 **A blow that takes time.** The server owns a deadline, not an action — which is why a bow stands still for ten seconds and a spoiled swing has no way to say so. `CombatAction` as a committed, interruptible process: commit → sustain → resolve, the round taken at the nock, interruption as a declared condition/effect table rather than a flag, and one new packet (the *end*, which never existed). Six phases; the polearm's reach and overwatch are the last two, and they are unbuildable before the first. |
