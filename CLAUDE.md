...
| [`docs/interiors.md`](docs/interiors.md) | **A building, and what it lets you see.** Three asks, one seam — the storey as a person's choice rather than `UpdateMaxDrawZ`'s, a closed building that keeps its inside to itself, and walls cut to the knee. The predicate all three change lives in one module, and the plan says which of its four readers each phase joins. |
