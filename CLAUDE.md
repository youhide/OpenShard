...
| [`docs/map/`](docs/map/) | 🚩 **The map, and everyone who reads it.** The pathfinding graph, the building flood, the minimap cache — five systems that each bake something off terrain and each key that bake to a file's mtime. Its [`README.md`](docs/map/README.md) is the index, and [`new_map_representation/`](docs/map/new_map_representation/) is the track that changes what terrain *is*: a world we can edit, an imported base with committed patches, one revisioned snapshot every reader holds. |
