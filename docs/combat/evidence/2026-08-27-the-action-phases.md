# A blow that takes time: the phases, and what each one found

*A record, not a status. Written as the `CombatAction` object was built out
(2026-08-27 onward) and kept verbatim; where it disagrees with the tree, the
tree is right.*

**How to read the numbering.** `Ф1`–`Ф7` are this record's phases and several
comments in the tree cite them by number. `D1`–`D12` are the decisions they were
built against, and those are next door in
[`design_actions.md`](../design_actions.md) — a `D`-number quoted below is that
document's. The phases still open — Ф5, Ф6 and the rest of Ф7 — are
[`plans/combat/actions/PLAN.md`](../../../plans/combat/actions/PLAN.md); what is
open across the domain is ranked in [`README.md`](../README.md).

## Before the model: the deadline it replaced

*Kept because every phase below is read against it.*
...
> three passes — there is no ranged half of the tick any more.

### The root cause, in one sentence

**The server owned a deadline, not an action.**

The only authoritative value was the scalar `next_swing: WorldTick` inside `Combat`
(`crates/server/state/src/components.rs:2116`). `SwingWindup` (`components.rs:2352`)
was not a process — it was a marker meaning *"this one has already been animated"*,
read once at `lib.rs:818` so `swings` did not animate twice. Four consequences,
each visible in the code of the day:

- **Preconditions were tested twice, at two different moments, and the second test
  had no way to say so.** `prepare_swings` checked reach and sight to start the
  gesture; `swings` checked them again at impact and on failure did a bare
  `continue` (`lib.rs:804`). The player watched a full swing and got neither a blow
  nor a reason.
- **The action had no end.** A target that died mid-swing was cleared, and the
  stretched animation on the client ran out its promised duration over an empty
  tile. A telegraph that cannot be cancelled is worse than none.
- **The two ends disagreed about interruption.** The client cancelled a one-shot when
  the body stepped (`design_fight_loop.md`'s D7); the server had never heard of the
  step and landed the blow anyway.
- **Melee and ranged were two admission rules in two tick functions**, so *reach* was
...
**Ф7 — arming, and the watches. 🚧 The target-in-sight slice.** The targeted
archery half commits when sight alone is cut, pays the configured
