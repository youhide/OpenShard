...
> superseded — see [`map_rebuild.md`](../../archive/world/map_rebuild.md).

The plan behind [`overview.md`](../research/a_map_we_can_change.md), with the mechanics in
[`mechanics.md`](../design_snapshot.md). Seven directions and one deferred. The first two
...
executable plan: [`snapshot.md`](2026-08-25-one-world-one-door.md), which grew a third phase in the
...
> [`snapshot.md`](2026-08-25-one-world-one-door.md)'s phase 1; what it left behind is that
> document's [backlog](2026-08-25-one-world-one-door.md#what-phase-1-left-behind).
...
> Being executed first, on its own: [`snapshot.md`](2026-08-25-one-world-one-door.md).
...
- The crate exists: [`openshard-map`](../../../crates/common/map/src/lib.rs),
  ...
  struck — see [`map_rebuild.md`](../../archive/world/map_rebuild.md)'s R1.) B fills it with the
...
> [`what_a_change_costs.md`](../../../plans/world/what_a_change_costs/PLAN.md). This direction is its S2
...
- Replace the file stamp in [`bake.rs`](../../../crates/common/movement/src/bake.rs#L22)
  ...
  [`minimap_lod_plan.md`](../design_minimap_lod.md), which already asks for exactly
  this key.
...
- **The local rebuild is no longer "later" for one of the three bakes, and it has
  ...
  - the **span layer** is 115.4 ms of it, at *both* ends, and is
    [`navigation_spans.md`](2026-08-25-the-span-layer.md#n8--the-bake-follows-a-patch)'s
  ...
  - the **coarse graph** is the 11.6 s one, and its locality is what
    [`map_rebuild.md`](../../archive/world/map_rebuild.md)'s *"A tile of ground moved — rebake,
  ...
  - the **client's `chunk::apply`** is 16.3 ms and is not a bake at all — it is
    ...
    [`to_the_client.md`](../design_chunks_to_the_client.md#backlog), where E left it.
- **Nothing here may be answered with a ground overlay.**
  [`map_rebuild.md`](../../archive/world/map_rebuild.md) refuses one by name and on three counts,
...
> [`to_the_client.md`](../design_chunks_to_the_client.md) — five phases, the measurements the
...
> [`map_rebuild.md`](../../archive/world/map_rebuild.md): the runtime map (R) and the search over
