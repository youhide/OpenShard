...
| | |
|---|---|
| [`overview.md`](../research/a_map_we_can_change.md) | **Start here.** What we want, and what is wrong today — stated as facts about this codebase, not as a wish list. Short on purpose. |
| [`mechanics.md`](../design_snapshot.md) | How a changeable map works: base, patch, snapshot; chunks; what goes stale; how a chunk reaches the client. Where a decision is not made, it says so and names what would settle it. |
| [`plan.md`](2026-08-25-seven-directions.md) | Seven directions with the code each one touches, in order, with what "done" means for each, plus one deferred on purpose. A0 and A are refactors with no feature in them. |
| [`snapshot.md`](2026-08-25-one-world-one-door.md) | 🚩 **The plan being executed first.** Directions A0 and A on their own — the block order gets a type, and the map gets one revisioned owner. No format, no patches, no network. Start a session here. |
| [`to_the_client.md`](../design_chunks_to_the_client.md) | Direction E on its own: the pipe, chosen off measurements rather than preference, and five phases from "the client's world is a parameter" to "an operator types `.setland` and a connected screen changes". **All five built.** |
| [`what_a_change_costs.md`](../../../plans/world/what_a_change_costs/PLAN.md) | 🚩 **The plan being executed now.** What era S has left once A0, A, B, C and E are in: direction D in full, plus the four things B, C and E left standing and the one the track has borrowed since its first line. Six nodes — one version 2 of the file, products keyed by the chunk they were built from, a block replaced where it stands, a folded log, revert as a verb, and the `tiledata`/multis a shard still takes from somebody's install. |
| [`client_today.md`](2026-08-25-the-clients-map-today.md) | What direction A takes a handle to, measured: the layout `WorldMap` actually has, what each bake costs in memory and on disk, and the ranked backlog found while inventorying the readers. |
...
[`handoffs/`](.) — one file per session that moved the track, newest
...
[`snapshot.md`](2026-08-25-one-world-one-door.md) is built, all three phases of it: the block order
...
[handoff](2026-08-24-the-ground-moves-while-people-stand-on-it.md).

**This track is era S of [`map_rebuild.md`](../../archive/world/map_rebuild.md)**, which is the
...
**E has started**, and its plan is [`to_the_client.md`](../design_chunks_to_the_client.md). The
...
decisions and the leftovers are in the [handoffs](.); the plan itself
...
[`what_a_change_costs.md`](../../../plans/world/what_a_change_costs/PLAN.md). The representation works, and
