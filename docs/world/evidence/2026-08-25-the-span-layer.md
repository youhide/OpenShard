# The span layer, node by node

The account of how the span layer was built: what each node cost, what each one
measured, and what each one saw and did not fix. It is a record — the model it
produced is [`design_spans.md`](../design_spans.md), and what is still open is
the domain [`README.md`](../README.md). Nothing here is a queue.

Nine nodes ran between 2026-08-18 and 2026-08-25. N0 to N4, N3b, N7 and N8 are
built; N5 and N6 are gated on measurements nobody has asked for.
...
for the reason [terrain_seam](../research/terrain_seam.md#-a-is-not-what-a-search-spends-its-time-on)
...
**Done.** [Above](../design_spans.md#what-the-facet-actually-holds).
...
The 1,462 ns [`terrain_seam.md`](../research/terrain_seam.md#what-one-search-costs)
records is the same measurement on a different run; the trio above is internally
...
**Built.** Needed N2, and [`terrain_seam.md`](../research/terrain_seam.md)'s E for the other
...
[`terrain_seam.md`](../research/terrain_seam.md#what-one-search-costs) recorded, 37,248
destinations each:
...
> [the handoff](2026-08-23-the-ground-and-its-bake-are-one-value.md).
...
**Built**, and with it [`terrain_seam.md`](../research/terrain_seam.md)'s F is answered: the
...
[`spans`](../design_spans.md#the-shape) and `counts` are facet-wide packed runs in block order, so
...
[What the census decides](../design_spans.md#what-the-census-decides) already rejected once and for
...
- **and a facet patched into shape answers identically to the same facet baked
  ...
  [the bake's own comment](../design_spans.md#the-shape) says would be silent.
...
[`design_chunks_to_the_client.md`](../design_chunks_to_the_client.md#backlog)'s backlog;
...
- **Residency and tiling.** ~~The estimate is under 20 MB~~ — **measured at
  ...
  [direction G](2026-08-25-seven-directions.md#g--residency-and-size-deferred-on-purpose)'s
  either way.
...
- **N3b found: a node budget is not a tile budget, and 400 was measured against
  ...
  [`terrain_seam.md`](../research/terrain_seam.md#what-one-search-costs), not this one.
...
- **`sight_clear`'s own height blindness.** The same class of defect — a sight
  ...
  [filed in `terrain_seam.md`](../research/terrain_seam.md#-blindterrain-stood-for-a-rule-that-cannot-exist)
...
- **The statics layout.** 120,745 allocations and 38.2 MiB where a CSR pair
  ...
  [direction B](2026-08-25-seven-directions.md#b--our-own-chunk-format-and-a-uo-importer)'s.
...
- **A client that gets its world off the wire had no coarse graph at all.
  ...
  **It needed the artifact to be named after its world.** One name per facet put
  ...
  same path — see [`navigation_artifact.md`](../reference/navigation_artifact.md)'s
...
**🚩 The node-expansion question is closed, and a session should not re-open it
without a new reason.** A node is ~200–210 ns of terrain against ~225 ns of A\*'s
own heap and hash. Four separate attacks on the terrain half were priced in one
run of `step_cost` — a full per-span record (~15 MB, 5% of a node in open
country and 19% at the castle), a rejection mask (1.6 MB, under 2%), a locality
hoist across the `MapTerrain`/`Spans` seam (no bytes, ~4%), and the dense
`average_land_z` before them — and **none of them moves a node by a tenth**. The
terrain half is ~18 ns of rule eight times, which is what N1's three tiers
already reduced it to. The lever that is left is not a cheaper node but **fewer
nodes**, and that is the coarse graph, which is built and which the shard reads.
What the entry leaves standing is the packed static record, under
[direction B](2026-08-25-seven-directions.md#b--our-own-chunk-format-and-a-uo-importer)'s
own gate and not this layer's.
