...
| *"the re-run is owed"* — `boat_step_cost`, in [`terrain_seam.md`](../research/terrain_seam.md) | Run 2026-08-22. 110 ns against 123 ns a step; a moored ship costs 13 ns and 12%, where the reading over the `Sea` double said 267% |
...
| ~~**The world map draws at LOD 0**~~ — **spent, and it was already spent when this table was written.** [`radar.md`](../design_radar.md)'s R0–R7 gave every window a level of its own, one `RadarView`, a swept coarse floor and a CPU byte budget that evicts. What the entry asked for — "a coarse producer that samples `WorldMap` directly" — is `build_chunk` at any level, and R8 has since taken it back out of the *schedule*: it cost 232 ms in one frame at the top of the ladder. The map is walked at `SWEEP_LOD` and the 151 products above it are reduced from it | Nothing. The entry is the fifth of this session's four decayed ones — see the table above, and read `radar.md`'s section 10 for what is actually open there |
