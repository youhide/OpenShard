...
> a house is a patch or a layer, in [`map_rebuild.md`](../archive/world/map_rebuild.md)'s R3.

The mechanics behind [`overview.md`](research/a_map_we_can_change.md). This document says what
has to be true and what the real choices are; where a decision is not made yet
it says so and names what would settle it. [`plan.md`](evidence/2026-08-25-seven-directions.md) is the work
itself.
...
[`map_rebuild.md`](../archive/world/map_rebuild.md)'s *"the map, in one type"* means: the
...
- **🚩 And the base is ours, which the stock client is not.** A 2D client and
  ...
  ours — [`map_rebuild.md`](../archive/world/map_rebuild.md) puts it plainly, *"the classic 2D
...
[direction F](evidence/2026-08-25-seven-directions.md#f--the-editor)'s *"committing a house into the base as its
...
- **Size: 64×64 tiles**, which is eight map blocks square, so no chunk boundary
  ...
  [`overview.md`](research/a_map_we_can_change.md) refuses that argument by name, since thrift is
  ...
  about the size. The numbers are in [`client_today.md`](evidence/2026-08-25-the-clients-map-today.md).
...
[`minimap_lod_plan.md`](design_minimap_lod.md) already states this contract from
