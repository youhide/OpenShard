# The roadmap: what is left of it

**This directory is a holding area, not a status page.** It is what the roadmap
still holds for the domains that have not been migrated to the documentation
canon yet. Each file below is a phase record — what was built, and the technical
context that came with it — and each is claimed by the batch that migrates its
domain, which moves it into that domain's `evidence/`.

Two things do not live here any more:

- **The order of work** is [`plans/roadmap/PLAN.md`](../../plans/roadmap/PLAN.md).
...
- **The status of a migrated domain** is that domain's README:
  ...
  [`render/`](../render/README.md). The phases that used to describe those three
...
| 1. Protocol | [`01-protocol.md`](01-protocol.md) | `protocol/` |
...
- [`backlog/protocol.md`](backlog/protocol.md) — why login encryption is
  deferred.
