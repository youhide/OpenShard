...
**D6 — The overlay is a dev-HUD toggle, off by default.** Beside `show_terrain`
and `show_occluders` in the World tab, and paid for only while it is on. It is
not a player-facing feature and no gameplay reads it.

## Phases

**All four are built.** What follows is the plan as it was taken, with each
