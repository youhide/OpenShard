# Replacing Graphic + Hue: the stages, and where each one got to

> **This is a record.** It is the staged plan and the running progress log that
> lived in `docs/item_kind.md`, kept as it was written. The model it produced is
> [`../design_item_kind.md`](../design_item_kind.md) — where the two differ, the
> design is right — what is not built is
> [`plans/items/item_identity/PLAN.md`](../../../plans/items/item_identity/PLAN.md),
> and what is still open is ranked in [`../README.md`](../README.md), which is
> the only status page for this domain.
