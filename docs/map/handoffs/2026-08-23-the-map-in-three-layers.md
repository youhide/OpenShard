...
| | |
|---|---|
| [`map_rebuild.md`](../map_rebuild.md) | the area's **entry point**: the three layers, the three eras, the decisions taken between the plans, and what each older document becomes |
| [`realtime_map.md`](../realtime_map.md) | **era R, executable**: what moves where, in which commit, with the DoD and the risk per node |
...
[`navigation_spans.md`](../navigation_spans.md), unchanged in substance. S is
[`new_map_representation/`](../new_map_representation/), resumed: the live
...
- **A house is the live layer, never a patch**, closing the last open row in
  [`mechanics.md`](../new_map_representation/mechanics.md). Its floors become
...
- **`plan.md`'s order is superseded past C**, and
  [`navigation_spans.md`](../navigation_spans.md)'s gate moved from
...
- **🚩 The search's node is a planar tile, and a column with two standing places
  ...
  [N3b](../navigation_spans.md#n3b--the-node-stops-being-a-tile), which the census
...
**R1, commit 1** — [`realtime_map.md`](../realtime_map.md#r1--the-table-leaves-the-file-reader).
