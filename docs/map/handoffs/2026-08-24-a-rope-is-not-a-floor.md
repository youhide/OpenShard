...
| **This client has no notion of a boat at all** | `clutter` expands no multis, so a ship is invisible to the client's step rule *and* to `predict_step`. The survey's median pier stands **seven** above the deck it boards onto, and a `0x22` carries no position: every boarding is seven units of disagreement at this end. `docs/client.md` does not mention boats |
