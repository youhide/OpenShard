...
| [`navigation_spans.md`](navigation_spans.md) | 🚩 **The first storey.** The layer HPA\* assumes underneath it and this engine never built: a column is a *list* of standable surfaces rather than one height, so a castle plateau is its own span instead of an island. Measured first — 1,462 ns a node expansion, of which A\*'s own machinery is noise, and 92.1% of columns hold no statics at all. Three tiers, under 20 MB, an oracle per node, and a measured ×4 against a ×6.4 ceiling. **N0 done and the rest deliberately not started** — it substitutes one argument of the call `terrain_seam.md`'s E is still shaping, so it waits for that plan to close rather than being written twice. |
