...
**Era P has started: N1, the span layer itself, is built, and N2, the step
rule that reads it, is proved equal to the map's over 248 M steps.**
...
[`navigation_spans.md`](navigation_spans.md): era R is over, N1 to N3b are
built — the shard walks on the span layer at 208 ns a node instead of 1,105, and
a node is now a *place to stand* rather than a tile — and **N4** (regions over
spans) is next, the repair a player would notice.
...
| [`navigation_spans.md`](navigation_spans.md) | 🚩 **The first storey — era P in full.** The layer HPA\* assumes underneath it and this engine never built: a column is a *list* of standable surfaces rather than one height, so a castle plateau is its own span instead of an island. Measured first — 1,462 ns a node expansion, of which A\*'s own machinery is noise, and 92.1% of columns hold no statics at all. Three tiers, an oracle per node, a measured ×4 against a ×6.4 ceiling. **N0–N3b are built** — the layer is 16.5 MiB, bakes in 0.07 s, equals `stand_surfaces` on all 29.4 M columns, answers what the map answers on 248,268,125 steps, and **the shard now walks on it**: a node expansion is 208 ns where it was 1,105 and a search from Britain's castle 0.168 ms where it was 0.793. N3b then spent that on the answer — a node is a place to stand, so 178 destinations that used to report an arrival are now the refusals they always were, and a route between two floors of one column exists. **N4 is next** — the visible repair, the castle plateau that is an island in the coarse graph. |
