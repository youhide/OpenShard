...
| A fragment's world position | Per pixel, in the shader: `shaders/statics.wesl`'s `fs_main` meets the view ray with the boxes the instance stands as (`nearest`), and `at = best.at` **is the world point this pixel is a picture of**. The lighting rebuild's [phase 6](../render/evidence/2026-08-11-lighting-rebuild-phases.md). This is what makes R3 a shader line rather than a re-cut of the art. |
...
[`design_frame_assembly.md`](../render/design_frame_assembly.md) would otherwise have us eyeballing.
...
[`pitfalls.md`](../render/evidence/pitfalls.md) is the ladder for anything else
...
- **The silhouette and outline passes** draw from the same fragments and will cut
  ...
  [`design_silhouettes.md`](../render/design_silhouettes.md) is about exactly that boundary.
