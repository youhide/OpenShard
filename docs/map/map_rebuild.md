...
| **D** | derived data keyed by the source revision instead of by file mtimes — the navigation bake and the building flood **already carry a `MapRevision` and refuse themselves on a mismatch**; the occluder measurements and the radar do not, and the radar's revision dimension has no production writer at all. **Now executable**, as S2 and S3 of [`what_a_change_costs.md`](new_map_representation/what_a_change_costs.md), which sharpens the key: a *facet* revision moves for all 7,168 chunks at once, so a product is keyed by the hash of the chunk it was built from |
