...
| ~~**D**~~ | **Built, as S2 and S3 of [`what_a_change_costs.md`](new_map_representation/what_a_change_costs.md).** A *facet* revision moves for all 7,168 chunks at once, so the answer is a **carry**: `RadarCache::moved` keeps every product no touched chunk lies under, and the three facet-wide artefacts a publish used to rebuild or drop now follow it over the chunks it named — the span index (0.3 ms), `WorldMap`'s statics (0.4 ms) and the coarse graph (80 ms against 28 s). The occluder measurements, keyed to file mtimes, are the one reader with no revision at all |
| ~~**E**~~ | **Built, all five phases.** Whole chunks to our client over the `0xBF` envelope in the `0xE000` range, deflated — [`to_the_client.md`](new_map_representation/to_the_client.md). The client's world is a parameter, it takes a facet it has no files for off the wire, keeps it under a `WorldId`, and is told when the shard's ground moves under it |
