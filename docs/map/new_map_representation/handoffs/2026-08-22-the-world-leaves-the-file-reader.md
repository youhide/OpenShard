...
importer.** `Map` was the single representation of the world, and it was
declared inside `openshard-uofiles`, whose stated job is reading the client's
files.

Landed as [`snapshot.md`](../snapshot.md)'s **phase 3**, in two commits.
...
block order, moved whole and unchanged), `map` (`Map`, `LandCell`, `LandTile`,
`StaticItem`), and `snapshot` (`MapSnapshot`, `MapRevision` — what used to be
its `lib.rs`).
...
| `Map::load_facet` | `uofiles::map::read_facet` | tests, examples, diagnostics — as before |
...
trying to leave. Cost of the split, paid: `Map` needed a public construction
path, and `MapSnapshot::load_facet` had to move to the importer, so a
file-reading crate is what publishes revision 1. That is honest — an importer is
exactly the thing that mints a first revision.
...
**`Map::from_parts` owns the sort, and the decoder lost it.** The per-block sort
by tile was the loader's, and it is the invariant `statics_at` and
...
[`plan.md`](../plan.md)'s B: a chunk reader builds the same `Map` through the
same `from_parts`, which turns B's round-trip acceptance test into an assertion
about *bytes* rather than about two parallel representations that agree by
inspection.
...
extent for `Map::from_blocks` (deferred to B on the plan's own words) and the
`Command::Send` newtype once `link.rs` is quiet.
...
**`Map` is the world under the entities, not the whole world a player sees.**
Houses, items, doors and boats are laid over it as entities; the client's
`steer.rs` reads `cluttered` — the map with the shard's live items over it —
while `predict_step` read the bare `Map`. Phase 2 put both halves on one side of
the thread boundary; making them one rule is still nobody's task, and it is
worth naming before direction C starts publishing.
