# The newtype sweep: bare integers to raw-then-validated fields

Record of the multi-session sweep of `crates/common/protocol` that gave every
packet field a name and every client-supplied one a check to pass through. It
ran from the pilot (2026-07-30) to N-gate (2026-08-31) and is closed; the
decisions it settled — N1 through N11, the four classes and the allowlist — are
[`design_wire_types.md`](../design_wire_types.md). Below is the census it
started from, the pilot, and the amendment every stage forced, in the order they
were written.

## The census it started from

193 `pub <name>: u8|u16|u32|i8|…` fields in the crate's packet structs:
...
And what the seam did with them — `dispatch::create_character`, before the
pilot:
...
checked. A client that sent `strength = 255, dexterity = 255, intelligence =
255` got it. A client that sent `skill value = 255` got a skill at 2550. A
client that sent any `u16` got it as a skin hue, staff-only hues included.
