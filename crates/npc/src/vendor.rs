...
pub fn open_shop(state: &mut WorldState, connection: ConnectionId, serial: u32) -> bool {
    ...
    // The container first, then its contents, then the prices — the client
    // pairs the 0x74 lines with the 0x3C items by order, so the same walk
    // builds both.
    ...
    state.send(
        connection,
        encode_open_container(stock_serial.raw(), SHOP_GUMP, version),
    );
    state.send(
        connection,
