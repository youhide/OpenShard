...
mod tests {
    ...
    fn plaintext(text: &str) -> PlaintextPassword {
        PlaintextPassword(text.to_owned())
    }

    fn raw(text: &str) -> RawPlaintextPassword {
        RawPlaintextPassword(text.to_owned())
    }

    #[test]
    fn a_hash_verifies_its_own_password() {
        let phc = hash(&plaintext("hunter2"));
        assert!(verify(&raw("hunter2"), &phc));
    }

    #[test]
    fn a_hash_rejects_the_wrong_password() {
        let phc = hash(&plaintext("hunter2"));
        assert!(!verify(&raw("hunter3"), &phc));
    ...
    fn two_hashes_of_one_password_differ() {
        ...
        assert_ne!(hash(&plaintext("same")), hash(&plaintext("same")));
    }

    #[test]
    fn a_non_phc_credential_verifies_against_nothing() {
        assert!(!verify(&raw("plaintext"), "plaintext"));
        assert!(!verify(&raw(""), ""));
    }
}
