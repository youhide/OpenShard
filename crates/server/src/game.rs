...
///
/// Only reachable with no client files configured, where nothing has a height
/// anyway.
const START_Z_WITHOUT_A_MAP: i8 = 0;
...
pub struct Game {
    ...
    /// Where new characters appear, from config.
    start: (u16, u16),
}
...
impl Game {
    ...
    ///
    /// The configured x and y, with the height read from the map. The `z` is not
    /// configurable on purpose: the map already knows it, and a second source of
    ...
    fn start_position(&self) -> Point {
        ...
        let z = self
            ...
            .map_or(START_Z_WITHOUT_A_MAP, |cell| cell.z);
        Point::new(x, y, z)
    }
    ...
    pub fn walk(&mut self, player: &mut Player, request: WalkRequest, now: Instant) -> Reply {
        let mut reply = Reply::default();
        ...
        let outcome = match &self.terrain {
            Some(terrain) => player.walker.request(request, terrain, now),
            None => player.walker.request(request, &OpenWorld, now),
        };
        match outcome {
...
mod tests {
    ...
    /// Anywhere; with no map the height is zero and every step is allowed.
    ...
    #[test]
    fn a_player_starts_where_config_says() {
        let mut game = Game::new(START);
        let (player, _) = game.character_play(&play()).unwrap();
        assert_eq!((player.walker.position.x, player.walker.position.y), START);
    ...
    fn walking_acks_and_moves() {
        ...
        let reply = game.walk(&mut player, walk(0, Direction::South), Instant::now());
        assert_eq!(reply.packets, vec![vec![0x22, 0, 0x01]], "acked");
        assert_eq!(
            player.walker.position,
            Point::new(START.0, START.1 + 1, START_Z_WITHOUT_A_MAP),
            "one tile south of where it started"
        );
    }
    ...
    fn turning_acks_without_moving() {
        ...
        let reply = game.walk(&mut player, walk(0, Direction::North), Instant::now());
        assert_eq!(
            reply.packets,
            vec![vec![0x22, 0, 0x01]],
            "a turn is acked too"
        ...
        assert_eq!(
            (player.walker.position.x, player.walker.position.y),
            START,
            "but does not move"
        );
    ...
    fn an_out_of_sequence_walk_is_rejected_with_the_real_position() {
        ...
        let reply = game.walk(&mut player, walk(9, Direction::South), Instant::now());
        ...
        assert_eq!(&reject[2..4], &START.0.to_be_bytes());
        assert_eq!(&reject[4..6], &START.1.to_be_bytes());
        assert_eq!(reject[7] as i8, START_Z_WITHOUT_A_MAP);
    }
    ...
    fn a_player_can_walk_a_full_lap_of_sequence_numbers() {
        ...
        //
        // The clock has to advance. Three hundred steps in the same instant is
        // not a sequence problem, it is a speedhack, and `WalkPace` is right to
        // refuse it — which is what this test found the moment the pace landed.
        let mut game = Game::new(START);
        let (mut player, _) = game.character_play(&play()).unwrap();
        ...
        for step in 0..300u32 {
            ...
            let reply = game.walk(&mut player, walk(sequence, Direction::South), now);
            assert_eq!(
    ...
    #[test]
    fn a_client_that_floods_walk_packets_is_throttled() {
        // The thing the pace exists for. Without it a client walks as fast as it
        // can send, which is a speedhack in one line of Python.
        let mut game = Game::new(START);
        let (mut player, _) = game.character_play(&play()).unwrap();
        let now = Instant::now();
        ...
        let mut refused = 0;
        for step in 0..200u8 {
            let reply = game.walk(&mut player, walk(step, Direction::South), now);
            if reply.packets[0][0] == 0x21 {
                refused += 1;
            }
        }
        assert!(refused > 150, "only {refused} of 200 instant steps refused");
    }
}
