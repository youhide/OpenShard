...
//!
//! Every one is ported reference data — ServUO's `Data/bodyTable.cfg`,
//! `SkillInfo.Table`, the names and `BaseSoundID`s off
...
//!
//! - **A `const` slice**, for the tables that are searched. `body_type`
//!   binary-searches its own on the tick path; `SKILLS` is indexed
...
fn quests(text: &str) -> String {
    ...
    for quest in &quests {
        ...
        writeln!(out, "            key: QuestKey::new({}),", owned(&quest.key)).unwrap();
        for (field, text) in [
            ("title", &quest.title),
