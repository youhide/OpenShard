...
/// The doc over the generated `crops::shipped`.
const CROPS_DOC: &str = "\
/// Every crop field the shard ships, built fresh from `data/crops.json`.
///
/// Ported from the `<spawning>` blocks of ServUO's `Regions.xml` — the farm
/// fields outside the towns, which spawn *objects* rather than creatures and so
/// never reached `spawns.json`.
///
/// **Keyed by an admin verb**, like `spawner::shipped`, and by the same one: a
/// facet's cotton is part of its population, so `populate:felucca` lays the
/// fields alongside the spawn regions and the townsfolk.
";

/// One set in `data/crops.json`.
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct CropFile {
    /// The admin verb that lays it.
    verb:   String,
    /// Which facet every field in the set belongs to.
    facet:  u8,
    /// The fields themselves.
    fields: Vec<CropFieldDef>,
}

/// One field of one crop.
///
/// The seconds are the data's rather than the ticks the engine runs on: ServUO
/// writes this pace as `minSpawnTime`/`maxSpawnTime` in an ISO duration, and a
/// tick count in the file would be a number nobody can check against upstream.
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct CropFieldDef {
    /// ServUO's own region name, kept for the log line and for reading the file.
    name:         String,
    /// Which crop grows here.
    crop:         String,
    /// West edge.
    x:            u16,
    /// North edge.
    ...
    /// Height in tiles.
    height:       u16,
    /// How many plants the field keeps standing.
    max_count:    u16,
    /// Seconds to wait after planting one before the next.
    ///
    /// ServUO draws each object's wait uniformly between `minSpawnTime` and
    /// `maxSpawnTime`, which is `PT10S`..`PT30S` for every farm field it ships;
    /// one delay here takes the middle of that range rather than inventing a
    /// second randomiser beside the one that picks the tile.
    respawn_secs: u64,
}

/// The `CropKind` a data name means. Named rather than numbered, and checked
/// here, so a typo is a build failure that says which field carries it instead
/// of a crop that silently becomes cotton.
...
/// `data/crops.json` into the `shipped` constructor.
fn crops(text: &str) -> String {
    let file: CropFile = serde_json::from_str(text).expect("crops.json");

    assert!(!file.verb.is_empty(), "a crop set with no verb can never be laid");
    assert!(
        !file.fields.is_empty(),
        "crop set {:?} lays no fields at all",
        file.verb
    );
    ...
    for field in &file.fields {
        assert!(
            ...
            "the field {:?} is {}x{}, which contains no tile to plant on",
            field.name,
            field.width,
            field.height
        );
        assert!(
            field.max_count > 0,
            "the field {:?} keeps no plants standing, so it does nothing",
            field.name
        );
        assert!(
            field.respawn_secs > 0,
            "the field {:?} regrows instantly, which is a plant per tick",
            field.name
        );
    }

    // **Fields of one crop may not overlap.** A field counts what is standing in
    // it by asking which plants are inside its box, because a plant carries no
    // field id — it is scenery, not a spawn, and it is not saved. Two boxes over
    // one patch of cotton would each count the other's plants and neither would
    // ever replant. Different crops may share ground: a count only ever looks at
    // its own.
    for (i, a) in file.fields.iter().enumerate() {
        for b in file.fields.iter().skip(i + 1) {
            let apart = a.crop != b.crop
                || a.x + a.width <= b.x
                || b.x + b.width <= a.x
                || a.y + a.height <= b.y
                || b.y + b.height <= a.y;
            assert!(
                apart,
                "the fields {:?} and {:?} both grow {} over the same ground, and each would \
                 count the other's plants as its own",
                a.name, b.name, a.crop
            );
        }
    }

    let mut out = String::from("// @generated by build.rs from data/crops.json.\n\n");
    out.push_str(CROPS_DOC);
    out.push_str("#[must_use]\npub fn shipped() -> Vec<CropSet> {\n    vec![CropSet {\n");
    writeln!(out, "        verb: {:?}.to_owned(),", file.verb).unwrap();
    out.push_str("        fields: vec![\n");
    for field in &file.fields {
        // Through the constructor, like a `Spawner`: the live planting timer is
        // the engine's and a number written here would be a second source for it.
        out.push_str("            CropField::new(\n");
        writeln!(out, "                {:?}.to_owned(),", field.name).unwrap();
        writeln!(out, "                {},", crop_kind(&field.crop, &field.name)).unwrap();
        writeln!(
            out,
            "                crate::spawner::SpawnArea {{ x: {}, y: {}, width: {}, \
             height: {}, facet: openshard_protocol::world::Facet({}) }},",
            field.x, field.y, field.width, field.height, file.facet
        )
        .unwrap();
        writeln!(out, "                {},", field.max_count).unwrap();
        writeln!(
            out,
            "                {} * openshard_state::TICKS_PER_SECOND,",
            field.respawn_secs
        )
        .unwrap();
        out.push_str("            ),\n");
