...
mod tests {
    ...
    fn a_replayed_scenario_walks_the_same_body_the_bench_scripts() {
        ...
        let mut mobile = crowd.snap(None, start, body, Facing::walking(Direction::East), hue);
        ...
        while elapsed <= script.length {
            ...
            for step in replay.advance(frame) {
                mobile = match step.glided {
                    true => crowd.see(None, step.to, body, step.facing, hue),
                    false => crowd.snap(None, step.to, body, step.facing, hue),
                };
            }
