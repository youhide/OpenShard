//! The static-art posts that imply a generated functional door.
//!
//! These are ServUO `DoorGenerator`'s four tables.  The server uses them to
//! place a `DarkWoodDoor`; the offline client bake uses the same fact to close
//! the corresponding gap in its wall contour.  A generic `WALL` match is not
//! enough: it would mistake ordinary architectural gaps for doors.

/// Whether a graphic is the west side of an east/west doorway.
#[must_use]
pub fn is_west_frame(id: u16) -> bool {
    WEST_FRAMES.binary_search(&id).is_ok()
}

/// Whether a graphic is the east side of an east/west doorway.
#[must_use]
pub fn is_east_frame(id: u16) -> bool {
    EAST_FRAMES.binary_search(&id).is_ok()
}

/// Whether a graphic is the north side of a north/south doorway.
#[must_use]
pub fn is_north_frame(id: u16) -> bool {
    NORTH_FRAMES.binary_search(&id).is_ok()
}

/// Whether a graphic is the south side of a north/south doorway.
#[must_use]
pub fn is_south_frame(id: u16) -> bool {
